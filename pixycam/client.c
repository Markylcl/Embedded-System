...
#define SUBSCRIPTION_TOPIC0      "/cc3200/PIXYCmd"

/* Defining Publish Topic Values                                             */
#define PUBLISH_TOPIC0           "/cc3200/PIXY"
#define PUBLISH_TOPIC1           "/cc3200/colorcmd"
...
/*
    char data[8] = "abcdef";
    MQTTClient_publish(gMqttClient, (char*) PUBLISH_TOPIC0, strlen(
                                                       (char*)PUBLISH_TOPIC0),
                                                   (char*)data,
                                                   strlen((char*) data), MQTT_QOS_2 |
    ...
    for (;;)
    {
        ...
        case MSG_RECV_BY_CLIENT:{
            ...
            {
                playload = GetPlayload(queueElemRecv.msgPtr);
                //UART_PRINT(playload);
            }

            if(strcmp(playload, ackcmd) == 0){
                             block_type newmsg;
                             char tosend[10] = "";
                             char color[3];
                             char x[3];
                             char y[3];
                             if(read_pixyqueue(&newmsg)==0){

                                 sprintf(color, "%d", newmsg.color);
                                 sprintf(x, "%d", newmsg.x);
                                 sprintf(y, "%d", newmsg.y);
                                 strcat(tosend,color);
//                                 strcat(tosend,"   x-cord:");
//                                 strcat(tosend,x);
//                                 strcat(tosend,"   y-cord:");
//                                 strcat(tosend,y);
                             }
                             lRetVal = MQTTClient_publish(
                                     gMqttClient,
...
//    //command thread
//    pthread_t commandthread = (pthread_t) NULL;
//    struct sched_param compriParam;
//    pthread_attr_t      attrs;
//
//    pthread_attr_init(&attrs);
//    compriParam.sched_priority = 1;
//    retc = pthread_attr_setschedparam(&attrs, &compriParam);
//    if (retc != 0) {
...
//        while (1);
//    }
//    retc = pthread_create(&commandthread, &attrs, MqttSendThread, NULL);
